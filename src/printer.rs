...
pub fn print_report(report: &BinaryReport, blocks: &[f64]) {
    print_banner();
    ...
    println!("├──────────────────────────────────────────────────────────────────────────────┤");
    println!("│ Format:        {:<61} │", report.format.to_string().cyan());
    println!("│ Architecture:  {:<61} │", report.architecture);
    println!("│ Subsystem:     {:<61} │", report.subsystem);
    println!("│ File Size:     {:<61} │", format!("{} bytes ({:.2} KB)", report.file_size, report.file_size as f64 / 1024.0));
    println!("│ Entry Point:   {:<61} │", format!("0x{:X}", report.entry_point).yellow());
    ...
    println!("  Overall Shannon Entropy: {:0.3} / 8.000  [{}]", report.overall_entropy, entropy_badge(report.overall_entropy));
    ...
    if !blocks.is_empty() {
        ...
        println!("  [{}]", bar);
        println!("  {:^66}", "0% ───────────────────────── 50% ──────────────────────── 100%");
        println!("  Legend: {} Zeroes  {} Text/Sparse  {} Code  {} Dense  {} Packed/Encrypted",
            "░".blue().dimmed(),
            "▒".cyan(),
            "█".green(),
            "█".yellow(),
    ...
    if !report.sections.is_empty() {
        ...
        println!("  ┌────────────┬─────────────┬─────────────┬──────────┬─────────┬──────────────┐");
        println!("  │ Name       │ Virt Size   │ Raw Size    │ Flags    │ Entropy │ Status       │");
        println!("  ├────────────┼─────────────┼─────────────┼──────────┼─────────┼──────────────┤");
    ...
    if !report.interesting_strings.is_empty() {
        ...
        if report.interesting_strings.len() > 15 {
            println!("  ... and {} more matches found", report.interesting_strings.len() - 15);
        }
    }
